...
The activation function we had chosen for the trainable layers is \textit{ReLU}(Rectified Linear Unit), while for
...
\item Loading the random sound samples into the model: The Neural Network Engine also presents the \textit{inference}
	...
	\begin{itemize}
		...
		\item MEL Filterbank transformation
			...
			The \textit{MEL} scale is a representation of the sounds that can be heard and
			distinguished by the human ear. The \textit{MEL spectrogram} is a graphical representation
			...
			\begin{figure}[H]
				...
				\centerline{\captionof{Figure 4.12: MEL Spectrogram example}}
			\label{ms}
			\end{figure}
	...
	The output of the transformation is a \textit{ndarray} containing the MEL spectrogram
